...
class Flickr8kDataset(Dataset):
    ...
    def __getitem__tensor(self, index: int):
        ...
        cap_tens = torch.full((self.max_len,), self.pad_value, dtype=torch.long, device=self.device)
        cap_tens[:len(cap_toks)] = torch.tensor([self.word2idx[word] for word in cap_toks], dtype=torch.long,
                                                device=self.device)
